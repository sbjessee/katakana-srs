...
const seedKatakanaData = (database: Database.Database): void => {
  ...
  const insertKatakana = database.prepare(
    'INSERT INTO katakana (character, romaji, type) VALUES (?, ?, ?)'
  );

  const insertReview = database.prepare(
    'INSERT INTO reviews (katakana_id, srs_stage, next_review_date) VALUES (?, 0, CURRENT_TIMESTAMP)'
  );

  const insertMany = database.transaction((items: Array<{char: string, romaji: string, type: string}>) => {
